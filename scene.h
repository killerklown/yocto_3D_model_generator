...
struct shape {
    ...
    std::vector<vec4i> quads;      // quads indices
...
struct material {
    ...
    vec3f kd = {0.5f, 0.5f, 0.5f};     // diffuse color
...
inline vec2f eval_texcoord(const shape* shp, int ei, const vec4f& ew) {
    if (shp->texcoord.empty()) return {0, 0};
    if (!shp->points.empty()) {
...
texture* make_grid_texture(int width, int height, int tile = 64,
    const vec4b& c0 = {90, 90, 90, 255},
    const vec4b& c1 = {128, 128, 128, 255});
texture* make_bumpdimple_texture(int width, int height, int tile = 64);
